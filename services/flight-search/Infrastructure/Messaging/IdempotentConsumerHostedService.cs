...
public abstract class IdempotentConsumerHostedService : BackgroundService
{
    ...
    protected virtual ushort PrefetchCount => 20;
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation("[{Consumer}] listening on queue {Queue}", GetType().Name, _queueName);
    ...
    private async Task HandleDeliveryAsync(BasicDeliverEventArgs args, CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "Errore consumer {Consumer} per messaggio {MessageId}", GetType().Name, messageId);
